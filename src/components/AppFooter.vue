<template>
  <q-footer class="bg-transparent">
    <div class="row justify-center items-center q-py-md">
      <div class="col-md-9 col-12 flex items-center justify-center">
        <q-btn flat icon="ion-logo-instagram" href="https://www.instagram.com/harlows.bar/" target="_blank" aria-label="Instagram"/>
        <q-btn flat icon="ion-logo-facebook" href="https://www.facebook.com/people/Harlows-Bar/100093700941605/" target="_blank" aria-label="Facebook"/>
        <q-btn flat icon="ion-logo-whatsapp" href="https://api.whatsapp.com/send?phone=34692940844" aria-label="Whatsapp"/>
        <q-btn flat icon="ion-mail" href="mailto:info@harlows.bar" aria-label="Email"/>
        <q-btn flat icon="ion-navigate" href="https://maps.app.goo.gl/y8zXASHuxx2pLbyf9" target="_blank" aria-label="Google Maps"/>
      </div>
    </div>
  </q-footer>
</template>

<script setup>
</script>
